...
export default class extends React.Component {
    ...
    _subscribe(){
        ...
        console.log('subscribe_status in details'+ this.state.subscribe_status)
        var channel_id = global.channel_id;
        console.log('===id===' + global.channel_id);
        var user_id = global.user_id;
        console.log("===user_id===" + global.user_id);

        global.channel_data.map((channel_datas, index)=>{
            if(channel_datas.channel_id==global.channel_id){
                channel_datas.subscribe_status = !this.state.subscribe_status
            }
        })
        this.forceUpdate();
        console.log("global.channel_data in details++++++++++++++++++++++++", global.channel_data)
            ...
            <ScrollView>
                ...
                <Tabs tabBarUnderlineStyle={{ backgroundColor: '#171841' }}
                    ...
                    <Tab heading heading="VNS"
                        ...
                        {this.state.channelVideo.map((item, index)=>(
                            <View key={index} >
                                <TouchableOpacity onPress={() => this._goToDetails(item)}>
                                    <View style={Styles.channel}>
                                        <Image source={{ uri: item.image }} style={Styles.channelImg} />
                    ...
                    <Tab heading="BOD"
                        ...
                        {this.state.bodData.map((bodItme, index)=>(
                            <View style={Styles.videoCaption} key={index}>
                                <TouchableOpacity onPress={() => {
                                    NavigationService.navigate('PublicDetail')
                                }}>
                                    <Image source={{ uri: bodItme.bodDataAvator }} style={Styles.videoImg} />
                                </TouchableOpacity>
                                <View style={Styles.videoDetails}>
                                    <Text style={Styles.videoDescript}>{bodItme.bodDataAvatorName}</Text>
                                </View>
                            </View>
                        ))}
                    </Tab>
...
//                         </Content> */}
//                         {this.state.channelVideo.map((item, index)=>(
//                              <View key={index} style={{backgroundColor: 'rgba(146,83,200,0.05)'}}>
//                                     <TouchableOpacity onPress={() => this._goToDetails(item)}>
//                                         <View style={Styles.channel}>
//                                             <Image source={{ uri: item.image }} style={Styles.channelImg} />
//                                         </View>
...
//                         <View style={{paddingLeft: '5%', paddingRight: '5%', backgroundColor: 'rgba(146,83,200,0.05)'}}>
//                             <Text style={{fontSize: 17, marginBottom: 5, marginTop: 25}}>{this.state.aboutData}</Text>
//                         </View>
//                     </Tab>
//                     <Tab heading="BOD"
