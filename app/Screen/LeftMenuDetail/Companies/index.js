...
import React from 'react';
import { StatusBar, TouchableOpacity, ImageBackground, Share, Image, FlatList, Alert } from 'react-native'
import { Container, Header, Switch, Content, Icon, Text, Card, Button, View } from 'native-base'
...
export default class extends React.Component {
    ...
    _subscribe(item, index){
        var channel_id = item.channel_id;
        console.log('===id===' + channel_id);
        var user_id = global.user_id;
        console.log("===user_id===" + global.user_id);
        this.state.channel_data[index].subscribe_status=!item.subscribe_status;
        this.forceUpdate();

        global.channel_data = this.state.channel_data;
        console.log("global.channel_data", global.channel_data)
        api.subscribe(channel_id, user_id).then((res)=>{
            console.log('subscribe_response____');
            console.log(res);
        //    this._getSubscribe();
    ...
    render() {
        ...
        return<Container>
            ...
            <View style={Styles.comapnyCategory}>
                <TouchableOpacity onPress={()=> NavigationService.navigate('LeftMenuDetailVnsSchool') }>
                    <Image source={{ uri: "https://vns2.quickflik.co.uk/wp-content/uploads/2019/04/New-Project-5.png" }} style={Styles.videoImg}  />
                </TouchableOpacity>
                <TouchableOpacity onPress={()=> NavigationService.navigate('LeftMenuDetailVnsSpotlight')}>
                    <Image source={{ uri: "https://vns2.quickflik.co.uk/wp-content/uploads/2019/04/google-download-1.png" }} style={Styles.videoImg}  />
                </TouchableOpacity>
                <TouchableOpacity onPress={()=> NavigationService.navigate('LeftMenuDetailVnsOverview') }>
                    <Image source={{ uri: "https://vns2.quickflik.co.uk/wp-content/uploads/2019/06/New-Project-10-1-e1560418979654.png" }} style={Styles.videoImg} />
                </TouchableOpacity>
            </View>
            <Content contentContainerStyle={Style.layoutDefault}>
                ...
                <View style={Styles.videoInfoDetails}>
                    {/* <FlatList
                        ...
                        renderItem={({ item, index }) => (
                            <View style={Styles.videoCaption}>
                                ...
                                {console.log(item)}
                                <TouchableOpacity style={item.subscribe_status? Styles.subscribe_button: Styles.unsubscribe_button} onPress={() => this._subscribe(item, index)}>
                                    {item.subscribe_status?
                                        <Text style={{color: '#ffffff', fontSize: 12}}>SUBSCRIBE</Text>
                                        :                                
                                        <Text style={{color: '#ffffff', fontSize: 12}}>UNSUBSCRIBE</Text>
                    ...
                    /> */}
                    {this.state.channel_data.map((item, index)=>(
                        <View style={Styles.videoCaption} key={index}>
                                <TouchableOpacity onPress={() => this._goToChannel(item)}>
                                    <Image source={{ uri: item.logo }} style={Styles.videoImg} />
                                </TouchableOpacity>
                                <View style={Styles.videoDetails}>
